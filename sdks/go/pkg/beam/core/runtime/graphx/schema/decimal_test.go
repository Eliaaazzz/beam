...
func TestDecimal_NegativeScaleEncoding(t *testing.T) {
	// The scale is an unsigned 32 bit varint, so 1E+3 (unscaled 1, scale -3)
	// is the scale 0xfffffffd followed by the single byte unscaled value.
	type row struct{ D Decimal }
	...
	want := row{decimalOf("1", -3)}
	wantBytes := []byte{0x01, 0x00, 0xfd, 0xff, 0xff, 0xff, 0x0f, 0x01, 0x01}
	var buf bytes.Buffer
	if err := enc(want, &buf); err != nil {
		t.Fatalf("enc(%v) = %v, want nil error", want, err)
	}
	if got := buf.Bytes(); !bytes.Equal(got, wantBytes) {
		t.Errorf("enc(%v) = %x, want %x", want, got, wantBytes)
	}
	got, err := dec(bytes.NewBuffer(wantBytes))
	if err != nil {
		t.Fatalf("dec(%x) = %v, want nil error", wantBytes, err)
	}
	if d := cmp.Diff(want, got, cmp.Comparer(decimalEqual)); d != "" {
		t.Errorf("dec(%x) diff (-want, +got): %v", wantBytes, d)
	}
	if got, want := want.D.String(), "1000"; got != want {
		t.Errorf("String() = %q, want %q", got, want)
	}
	// A scale outside the 32 bit range, such as a signed 64 bit varint of -3,
	// fails to decode.
	bad := []byte{0x01, 0x00, 0xfd, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x01, 0x01, 0x01}
	if got, err := dec(bytes.NewBuffer(bad)); err == nil {
		t.Errorf("dec(%x) = %v, want error for an out of range scale", bad, got)
	}
	if got, want := (Decimal{Unscaled: big.NewInt(0), Scale: -2}).String(), "0"; got != want {
		t.Errorf("zero with negative scale String() = %q, want %q", got, want)
	}
}
