...
// shifted by the minimum int64 so that the byte order matches the
// chronological order. Encoding a value with sub millisecond precision fails;
// truncate such values with time.Time.Truncate first.
