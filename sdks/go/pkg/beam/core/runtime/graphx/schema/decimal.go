...
//
// The representation type is BYTES: the scale as an unsigned 32 bit varint,
// followed by the length prefixed two's complement big endian bytes of the
// unscaled value. Decimal can be a field of a row, but has no schema of its
// own and cannot be a top level element type.
type Decimal struct {
	Unscaled *big.Int
	Scale    int32
}
...
func encodeDecimal(d Decimal, w io.Writer) error {
	// The scale is an INT32 written as an unsigned 32 bit varint.
	if err := coder.EncodeVarUint64(uint64(uint32(d.Scale)), w); err != nil {
		return err
	}
...
func decodeDecimal(r io.Reader) (Decimal, error) {
	scale, err := coder.DecodeVarUint64(r)
	if err != nil {
		return Decimal{}, errors.Wrap(err, "decoding decimal scale")
	}
	if scale > math.MaxUint32 {
	...
	return Decimal{Unscaled: bigIntFromTwosComplement(b), Scale: int32(uint32(scale))}, nil
}

// twosComplementBytes returns the minimal big endian two's complement bytes
// of x. A nil x is zero.
