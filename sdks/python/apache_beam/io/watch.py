...
class _GrowthRestrictionTracker(iobase.RestrictionTracker):
  ...
  def try_claim(self, holder: list) -> bool:
    ...
    if stop:
      ...
      # new_outputs is sorted and all strictly above the old cursor, so the
      # last one is the new high-water mark; an empty round leaves it unchanged.
      new_cursor = restriction.cursor
      if new_outputs:
...
class Watch(PTransform):
  ...
  def with_timestamp_cursor(self) -> 'Watch':
    ...
    skipped, so out-of-order or equal-timestamp outputs are dropped. For sources
    that re-list arbitrary collections or surface outputs out of event-time
    order, use the default exact (hash) dedup instead.
    """
